...
Module HIDDLLInterface
    ...
    Private Const WM_HID_EVENT As Decimal = WM_APP + 200 '32968
    ...
    Private USBclass As USB

    ' Set up a windows hook to receive notification
    ' messages from the HID controller DLL - then connect
    ' to the controller
    Public Function ConnectToHID(ByVal targetForm As System.Windows.Forms.Form, ByVal usb As USB) As Boolean
        USBclass = usb

        Dim pHostWin As Integer = targetForm.Handle.ToInt32
        FWinHandle = pHostWin
        pHostWin = hidConnect(FWinHandle)
        FPrevWinProc = DelegateSetWindowLong(FWinHandle, GWL_WNDPROC, Ref_WinProc) ' por este delegado es que no se muestra la aplicacion como deberia ser
        HostForm = targetForm
